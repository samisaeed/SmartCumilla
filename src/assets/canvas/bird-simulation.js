...
Bird.obj = Bird.def(
    function() {
        this.vtr = new Bird.Vtr(),
            this.accel, this.width = 1200, this.height = 800, this.depth = 700, this.ept, this.area = 200,
...
function draw() {
    ...
    Bird.L = new Bird.Vtr(0, 20000, 50000);
    Bird.V = new Bird.Vtr(0, 0, 50000);
    var birds = [];
    var b = [];
    for (var i = 0; i < 500; i++) {
        _b = b[i] = new Bird.obj();
        _b.pos.x = Math.random() * 800 + 800; // Adjust the x-coordinate to start from the right edge
        _b.pos.y = Math.random() * 800; // Adjust the y-coordinate to start from the top
        _b.pos.z = Math.random() * 800 - 400;
        _b.vel.x = Math.random() * -2 - 1; // Adjust the initial x-velocity to move towards the left
